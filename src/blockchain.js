...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve) => {
            ...
            // check if blockchain contains genesis block
            if(self.height == -1) {
     ...
     */
     submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            let verification_bool = bitcoinMessage.verify(message, address, signature)
            if (currentTime - time <  5*60) {
                if(verification_bool) {
                    ...
                    reject(Error("Not able to verify message."))
                }
            } else {
    ...
    validateChain() {
        ...
        return new Promise(async (resolve) => {
            self.chain.forEach(async(block, index) => {
                ...
                if(!isValid){
                    errorList.push(`block at index ${index} is invalid`);
                }
                if(index>=1) {
                    if(block.previousBlockHash != self.chain[index - 1].hash ) {
                        errorList.push(`block at index ${index} hash wrong previous hash value`);
                    }
                }
            });
            resolve(errorList)
